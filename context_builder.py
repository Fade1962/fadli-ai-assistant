import re
from config import MAX_CONTEXT_CHARS, RECENT_FILE_LIMIT
from storage import recent_uploads
...
def build_file_context(chat_id, query):
    ...
    for row in uploads:
        ...
        candidates.append((10000, row["file_name"], content[:1800]))
        for chunk in _chunks(content):
            score = len(query_tokens & _tokens(chunk))
            if score:
                candidates.append((score, row["file_name"], chunk))
